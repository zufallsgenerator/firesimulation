  ...
  CB.Class("Firems", {
    ...
    _makeColor: function (n) {
      return [
        255,
        Math.min(n * 1.2, 255),
        Math.min(255, Math.max(n - 175, 1) * 4),
        n,
      ];
    },
    ...
    tick: function (ctx, diff) {
      ...
      function processLine(target, lower, middle, upper) {
        ...
        for (x = 0; x < length; x++) {
          ...
          if (lower) {
            sum +=
              (leftbias * (lower[x - 1] || 0) +
                (lower[x] || 0) * centerbias +
                rightbias * (lower[x + 1] || 0)) *
              topbias;
          }
          sum +=
            (leftbias * (middle[x - 1] || 0) +
              rightbias * (middle[x + 1] || 0)) *
            middlebias;
          if (upper) {
            sum +=
              (leftbias * (upper[x + 1] || 0) +
                (upper[x] || 0) * centerbias +
                rightbias * (upper[x - 1] || 0)) *
              bottombias;
          }
